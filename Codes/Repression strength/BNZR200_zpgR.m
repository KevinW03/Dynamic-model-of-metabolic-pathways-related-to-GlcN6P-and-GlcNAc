...
function BNZR200_zpgR
% Strain BNZR2.00 after weakening zwf,pfkA, and glmM

close all

x = 9; % Nubmer of metabolites
m = 1000; % Nubmer of loops
v = 11; % Nubmer of reactions
repressing_zwf = zeros(m,1); % To save the repressinglevel of zwf
repressing_pfkA= zeros(m,1); % To save the repressing level of pfkA
repressing_glmM= zeros(m,1); % To save the repressing level of glmM
...
for i = 1:m
    ...
    zwf_DL = 0.97*rand(1); % Downregulating zwf
    pfkA_DL = 0.95*rand(1); % Downregulating pfkA
    glmM_DL = 0.77*rand(1); % Downregulating 
    par.Km = 1.5*rand(1,v)+0.5.*ones(1,v);
    par.vmax = ones(1,v);
    par.n = 4*rand(1);
    par.ki =rand(1);
    par.vmax(1) = 0.5;
    par.vmax(5) = 0.4*(1-zwf_DL); % HMP 
    par.vmax(6) = 1.9*(1-pfkA_DL); % EMP
    par.vmax(7) = 0.5*(1-glmM_DL); % PSP 
    par.vmax(11) = 0; % The synthetic pathway of GlcNAc 
    ...
    repressing_zwf(i,1) = zwf_DL;
    repressing_pfkA(i,1) = pfkA_DL;
    repressing_glmM(i,1) = glmM_DL;
...
writematrix( [repressing_zwf repressing_pfkA repressing_glmM GlcN6P_yield GlcNAc_yield],'yield.xlsx');
...
% Reaction v_4
vmax(4) = 2 + par.vmax(4)*(x(1)^par.n/(par.ki^par.n+x(1)^par.n));
v_4 = vmax(4)*x(3)/(x(3)+par.Km(4));
% Reaction v_5
