...
weaken_zwf = zeros(m,1); % To save the weaken level of zwf
weaken_pfkA= zeros(m,1); % To save the weaken level of pfkA
weaken_glmM= zeros(m,1); % To save the weaken level of glmM
...
for i = 1:m
    ...
    zwf_DR = rand(1); % Downregulating zwf
    pfkA_DR = rand(1); % Downregulating pfkA
    glmM_DR = rand(1); % Downregulating 
    par.Km = 1.5*rand(1,v)+0.5.*ones(1,v);
    par.vmax = ones(1,v);
    par.n = 4*rand(1);
    par.ki =rand(1);
    par.vmax(1) = 0.5;
    par.vmax(5) = 0.7*zwf_DR; % HMP 
    par.vmax(6) = 1.8*pfkA_DR; % EMP
    par.vmax(7) = 0.5*glmM_DR; % PSP 
    ...
    weaken_zwf(i,1) = zwf_DR;
    weaken_pfkA(i,1) = pfkA_DR;
    weaken_glmM(i,1) = glmM_DR;
...
writematrix( [weaken_zwf weaken_pfkA weaken_glmM GlcN6P_yield GlcNAc_yield],'yield.xlsx');
